...
class ForwardProblem(NonlinearProblem):
  ...
  def deflate(self, root):
    self.solutions.append(root.copy(deepcopy=True))
  ...
  def get_submat(self):
    ...
    if self.eqn_subindices is not None:
        Jmat = Jmat.createSubMatrix(self.eqn_subindices, self.var_subindices)

    if self.inact_subindices is not None:
        Jmat = Jmat.createSubMatrix(self.inact_subindices, self.inact_subindices)

    Pmat = None
    if self.Pmat is not None:
        Pmat = self.Pmat(self.y)
        if self.eqn_subindices is not None:
            Pmat = Pmat.createSubMatrix(self.eqn_subindices, self.var_subindices)

        if self.inact_subindices is not None:
            Pmat = Pmat.createSubMatrix(self.inact_subindices, self.inact_subindices)

    return (Jmat, Pmat)

  def create_vecs(self):
    left = as_backend_type(self._tmpvec1).vec().duplicate()
  ...
  def Jv(self, v, Jonv, x):
    self._subJ.mult(v, self._ptmpvec1)

    Jonv.zero()
    Jonv.axpy(self.deflation(), self._ptmpvec1)

    derivative = self.deflation_derivative()

    subres = as_backend_type(self.residual).vec().getSubVector(self.inact_subindices)
    subder = as_backend_type(derivative).vec().getSubVector(self.inact_subindices)
    Jonv.axpy(PETScVector(subder).inner(v), PETScVector(subres))
    as_backend_type(self.residual).vec().restoreSubVector(self.inact_subindices, subres)
    as_backend_type(derivative).vec().restoreSubVector(self.inact_subindices, subder)
...
class VIForwardProblem(ForwardProblem):
    ...
    def create_vecs(self):
      ...
      argh = left.getSubVector(self.inact_subindices)
      left_ = argh.duplicate()
      left.restoreSubVector(self.inact_subindices, argh)
      left = left_
