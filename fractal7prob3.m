...
% Newton with good approximate MSPIN jacobian
cp=zeros(size(x));
cit=zeros(size(x));
% run Newton's method for each starting guess
for i=1:npts
...
[res_mspinex, k,path_mspinex]=mynewt_mspinexact_path(2,2,tol);
[res_newt, k,path_newt]=mynewt_path(2,2,tol);
[res_mspinap1, k,path_mspinap1]=mynewt_mspinaprox1_path(2,2,tol);
[res_mspinap2, k,path_mspinap2]=mynewt_mspinaprox2_path(2,2,tol);
...
plot(1:length(res_mspinap2),res_mspinap2,'m--o')
xlabel('Iterate')
...
plot(r2(1),r2(2),'ro','Linewidth',1.5)
plot(r3(1),r3(2),'ro','Linewidth',1.5)
xlabel('x')
ylabel('y')
colorbar
title('log(||F_{MSPIN}(x) +1 ||)','FontSize',14)
...
% function for bad approximate mspin jacobian
...
x=b(1); y = b(2);
% evaluate function
fval=[(x-4*y^3 +3*y^2+27*y+7)^3- y^3; x+y + 1];
end
...
x = b(1); y = b(2);
% evaluate function
fval=[(x - 4*y^3 + 3*y^2 + 26*y +7); (4*y^3-3*y^2-25*y-6)];
end


function J_exact = Jex(b)
x = b(1); y = b(2);
J_exact= [3*(x-4*y^3 +3*y^2+27*y+7)^2, (3*(-12*y^2+6*y+27)*(x-4*y^3 +3*y^2+27*y+7)^2 - 3* y^2); 1 1];
end

function J_mspin = Jmspin(b)
x = b(1); y = b(2);
J_mspin= [1 (-12*y^2+6*y+26);  0 (12*y^2-6*y-25)] ;
end
